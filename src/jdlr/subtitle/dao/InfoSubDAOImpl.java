...
public class InfoSubDAOImpl implements InfoSubDAO {
	...
	public void addInfo(BDDInfo bddinfo) throws DAOException {
		...
		try {
			...
			preparedStatement = (PreparedStatement) 
					connection.clientPrepareStatement("INSERT INTO file_spec(line_number, line_min, line_text,"
							+ " file_id) SELECT ?, ?, ?, file_sub.id FROM file_sub WHERE file_sub.file_name = ?;");
			preparedStatement.setLong(1, bddinfo.getLine_number());
			preparedStatement.setString(2, bddinfo.getLine_min());
			preparedStatement.setString(3, bddinfo.getLine_text());
	...
	public List<BDDInfo> getAllBDDInfo() throws DAOException {
		...
		try {
			...
			result = statement.executeQuery("SELECT line_number, line_min, line_text, file_sub.file_name as file_name FROM"
					+ " file_spec JOIN file_sub ON file_spec.file_id = file_sub.id;");
